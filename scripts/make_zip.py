...
def  main():
  ...
  installer = "\installer\iPlugGen Installer.exe"
   
  if demo:
    installer = "\installer\iPlugGen Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\iPlugGen manual.pdf" 
  ]
  ...
  ZIPNAME = "iPlugGen-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "iPlugGen-v" + FULLVERSIONSTR + "-win-demo.zip"
