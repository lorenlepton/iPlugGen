...
if %1 == 1 (echo Making iPlugGen Windows DEMO VERSION distribution ...) else (echo Making iPlugGen Windows FULL VERSION distribution ...)
...
msbuild iPlugGen.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild iPlugGen.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\iPlugGen.aaxplugin\Contents\Win32\iPlugGen.aaxplugin --out .\build-win\aax\bin\iPlugGen.aaxplugin\Contents\Win32\iPlugGen.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\iPlugGen.aaxplugin\Contents\x64\iPlugGen.aaxplugin --out .\build-win\aax\bin\iPlugGen.aaxplugin\Contents\x64\iPlugGen.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\iPlugGen.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\iPlugGen.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "iPlugGen Installer" ".\installer\iPlugGen Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\iPlugGen Installer.exe" ".\installer\iPlugGen Demo Installer.exe"
REM -del ".\installer\iPlugGen Installer.exe"
REM -)
